...
def encode_boxes(
    ...
    reference boxes
    Args:
        reference_boxes (torch.Tensor): reference boxes
        proposals (torch.Tensor): boxes to be encoded
        weights (torch.Tensor[4]): the weights for ``(x, y, w, h)``
    """
...
class BoxCoder:
    ...
    def encode(
        self,
        reference_boxes: list[torch.Tensor],
    ...
    def encode_single(
        self, reference_boxes: torch.Tensor, proposals: torch.Tensor
    ) -> torch.Tensor:
        """
        Encode a set of proposals with respect to some
        reference boxes
    ...
    def decode(
        self, rel_codes: torch.Tensor, boxes: list[torch.Tensor]
    ) -> torch.Tensor:
        assert isinstance(
            boxes, (list, tuple)
        ...
        ), "This function expects rel_codes of type torch.torch.Tensor."
    ...
    def decode_single(
        self, rel_codes: torch.Tensor, boxes: torch.Tensor
    ) -> torch.Tensor:
        """
        From a set of original boxes and encoded relative box offsets,
        get the decoded boxes.
        Args:
            rel_codes (torch.Tensor): encoded boxes
            boxes (torch.Tensor): reference boxes.
        """
        ...
        c_to_c_h = pred_h * torch.tensor(
            0.5, dtype=pred_ctr_y.dtype, device=pred_h.device
        )
        c_to_c_w = pred_w * torch.tensor(
...
class Matcher:
    ...
    def __call__(self, match_quality_matrix: torch.Tensor) -> torch.Tensor:
        """
        Args:
            match_quality_matrix (torch.Tensor[float]): an MxN tensor, containing the
            pairwise quality between M ground-truth elements and N predicted elements.
        Returns:
            matches (torch.Tensor[int64]): an N tensor where N[i] is a matched gt in
        ...
        if self.allow_low_quality_matches:
            assert all_matches is not None, "all_matches should not be None"
